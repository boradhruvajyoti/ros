...
interface FranchiseStats {
  totalFranchiseNetworkRevenue: number;
  totalRoyaltiesCollectedMonth: number;
  marketingFundPool: number;
  activeFranchiseLocations: number;
  avgQsrAuditScore: string;
}
...
export default function FranchisePage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => apiGet<{ franchises: FranchiseOutlet[]; stats: FranchiseStats }>('/franchise/overview'),
  });

  const outlets: FranchiseOutlet[] = (data?.franchises || []).map((o) => ({
    ...o,
    status: settledIds[o.id] ? 'SETTLED' : o.status,
    lastSettlement: settledIds[o.id] ? 'Just now' : o.lastSettlement,
  }));

  const brandNames = Array.from(new Set(outlets.map((o) => o.brand).filter(Boolean)));
  ...
  const handleSettle = (id: string) => {
    setSettledIds((prev) => ({ ...prev, [id]: true }));
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            Multi-branch royalty aggregation, centralized marketing fund reconciliation, and automated GST-compliant billing.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          {brands.length > 1 && (
            ...
            >
              {brands.map((b) => (
                <option key={b} value={b}>{b === 'ALL' ? '🏢 All Branches / Brands' : b}</option>
              ))}
            </select>
          )}
          <button 
            onClick={() => alert('Exporting live royalty schedules as CSV/XLSX for HQ ERP integration...')}
      ...
      {isLoading ? (
        <div className="p-12 text-center text-slate-400 flex flex-col items-center justify-center gap-3">
          ...
          <p className="text-sm font-medium">Loading Franchise Ledger & Branch Data...</p>
        </div>
      ) : (
        <>
          {/* KPI Metrics */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-slate-900/60 border border-slate-800/80 rounded-2xl p-5 backdrop-blur-xl">
              ...
              <div className="mt-3">
                <div className="text-2xl font-bold text-white">₹{totalGrossSales.toLocaleString('en-IN')}</div>
                <div className="text-xs text-emerald-400 flex items-center gap-1 mt-1 font-medium">
                  <ArrowUpRight className="w-3.5 h-3.5" /> Live Audited POS Volume
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-900/60 border border-slate-800/80 rounded-2xl p-5 backdrop-blur-xl">
              ...
              <div className="mt-3">
                ...
                <div className="text-xs text-amber-400/90 mt-1">
                  {filteredOutlets.filter((o) => o.status !== 'SETTLED').length} Outlets Pending
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-900/50 border border-slate-800/80 rounded-2xl overflow-hidden backdrop-blur-xl shadow-xl">
            <div className="px-6 py-4 border-b border-slate-800 flex items-center justify-between">
              ...
              <span className="text-xs font-medium text-slate-400">
                Showing {filteredOutlets.length} Active Locations
              </span>
            </div>
            ...
            {filteredOutlets.length === 0 ? (
              <div className="p-12 text-center text-slate-400 space-y-2">
                ...
                <p className="font-semibold text-white">No Franchise Outlets Registered</p>
                <p className="text-xs">Add additional branches to view multi-unit franchise royalty settlements.</p>
              </div>
            ) : (
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-slate-800 bg-slate-950/40 text-xs font-semibold text-slate-400 uppercase tracking-wider">
                      <th className="py-3.5 px-6">Outlet & Branch</th>
                      <th className="py-3.5 px-6">Location</th>
                      <th className="py-3.5 px-6 text-right">Gross Sales</th>
            ...
            <div className="p-6 space-y-4">
              <div className="p-4 rounded-xl bg-slate-950/50 border border-slate-800 space-y-2">
                <div className="flex justify-between text-xs text-slate-400">
                  ...
                  <span>LOCATION: <strong className="text-slate-200">{invoicingModal.city}</strong></span>
                </div>
              ...
              <div className="space-y-2.5 text-sm">
                <div className="flex justify-between text-slate-300">
                  <span>Gross Sales (POS Audited)</span>
              ...
              {invoiceSent ? (
                <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 text-xs flex items-center gap-2">
                  ...
                  E-Invoice dispatched to branch finance portal & registered mail!
                </div>
              ) : null}
