...
const INITIAL_ROBOTS: RobotUnit[] = [
  {
    id: 'ROBOT-01',
    name: 'Bella #1 (Fluffy)',
    model: 'BellaBot Pro (Purr Edition)',
    batteryPercent: 88,
...
export default function RoboticsPage() {
  ...
  const [selectedRobot, setSelectedRobot] = useState<RobotUnit>(INITIAL_ROBOTS[0]);
  ...
  const handleDispatch = (robotId: string) => {
    ...
    setDispatchToast(`Dispatched ${selectedRobot.name} to ${dispatchTable}! LiDAR path clearance active.`);
  ...
  const handleRecall = (robotId: string) => {
    ...
    setDispatchToast(`${selectedRobot.name} returning to kitchen dock.`);
    setTimeout(() => setDispatchToast(null), 3000);
  };

  const totalDeliveries = robots.reduce((acc, r) => acc + r.deliveredTripsToday, 0);
  const avgBattery = Math.round(robots.reduce((acc, r) => acc + r.batteryPercent, 0) / robots.length);
            ...
            <div className="space-y-3 pt-2">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-600 hover:to-blue-700 text-white text-xs font-bold shadow-lg shadow-cyan-500/20 transition"
                >
